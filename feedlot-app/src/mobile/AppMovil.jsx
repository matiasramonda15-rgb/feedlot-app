...
function Home({ usuario, nav }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      <Topbar titulo="Feedlot" sub={`Hola, ${usuario?.nombre || 'Empleado'}`} />
      <Scroll>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <Scroll>
        ...
        {[
          { label: 'Cantidad de animales', key: 'cantidad', placeholder: 'ej. 85', type: 'number' },
          { label: 'Kg medidos en báscula', key: 'kg_bascula', placeholder: 'ej. 20380', type: 'number' },
        ].map(f => (
          <div key={f.key} style={{ marginBottom: '.85rem' }}>
            <div style={{ fontSize: 11, fontWeight: 600, color: C.muted, textTransform: 'uppercase', letterSpacing: '.06em', marginBottom: 4 }}>{f.label}</div>
        ...
        {prom && prom < 180 && <div style={{ background: '#3D2A00', border: `1px solid ${C.amber}`, borderRadius: 8, padding: '.75rem', marginBottom: '.85rem', fontSize: 12, color: C.amber, lineHeight: 1.5 }}>⚠ Peso menor a 180 kg. Se creará alerta para segunda dosis de Alliance+Feedlot a los 20 días.</div>}
