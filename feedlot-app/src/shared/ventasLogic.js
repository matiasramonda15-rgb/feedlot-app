...
// corralesVenta: [{ corral_id, cantidad, kg_vivo }]
...
export async function registrarVenta(supabase, {
  ...
  const totalKgNeto = Math.round(totalKgBruto * (1 - desbastePct / 100) * 10) / 10
  const precio = parseFloat(precioKg) || 0
  const montoTotal = precio ? Math.round(totalKgNeto * precio * 100) / 100 : null
  const montoFact = precio ? (montoFacturado != null && montoFacturado !== '' ? parseFloat(montoFacturado) : montoTotal) : null
  ...
  for (const cv of validos) {
    const kgNetoCv = Math.round(parseFloat(cv.kg_vivo) * (1 - desbastePct / 100) * 10) / 10
    const montoTotalCv = precio ? Math.round(kgNetoCv * precio * 100) / 100 : null
    const montoFactCv = (precio && montoFact && totalKgNeto) ? Math.round(montoFact * kgNetoCv / totalKgNeto * 100) / 100 : montoTotalCv
    ...
    const { error } = await supabase.from('ventas').insert({
      ...
      total: montoTotalCv,
      monto_facturado: precio ? montoFactCv : null,
      monto_negro: precio ? montoNegroCv : null,
      iva_pct: precio ? ivaPct : null,
      iva_monto: (precio && montoFactCv) ? Math.round(montoFactCv * ivaPct / 100) : null,
