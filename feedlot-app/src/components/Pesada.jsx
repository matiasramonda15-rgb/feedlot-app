...
export default function Pesada({ usuario }) {
  ...
  async function guardarPesada() {
    if (!corralSel) { alert('Seleccioná un corral'); return }
    const total = ['A','B','C','D'].reduce((s, k) => s + (parseInt(form[k]) || 0), 0)
    if (total === 0) { alert('Ingresá al menos un rango'); return }
    setGuardando(true)

    const { data: pesada, error } = await supabase.from('pesadas').insert({
      corral_id: parseInt(corralSel),
      tipo: 'clasificacion',
  ...
  const proximaPesada = '04/05/2026'
      ...
      <div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: '1.5rem' }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 12, color: '#6B6760', fontFamily: "'IBM Plex Mono', monospace" }}>Clasificación por rangos de peso</div>
          </div>
        </div>
        ...
        <Card titulo="Corral a pesar">
          <select style={{ width: '100%', padding: '10px 12px', border: '1px solid #E2DDD6', borderRadius: 8, fontSize: 13, fontFamily: "'IBM Plex Sans', sans-serif", background: '#fff' }}
            ...
            {corrales.map(c => (
              <option key={c.id} value={c.id}>Corral {c.numero} · {c.rol} · {c.animales || 0} animales</option>
            ))}
          </select>
          {corral && (
            <div style={{ marginTop: 12, padding: '10px 12px', background: '#F7F5F0', borderRadius: 8, fontSize: 12, color: '#6B6760', fontFamily: "'IBM Plex Mono', monospace" }}>
              Total animales en corral: <strong>{corral.animales || 0}</strong>
            </div>
          )}
        </Card>

        <Card titulo="Distribución por rangos">
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 16 }}>
            {RANGOS.map(r => (
              <div key={r.key} style={{ background: r.color, border: `1px solid ${r.border}`, borderRadius: 8, padding: '1rem' }}>
                <div style={{ fontSize: 11, fontWeight: 600, color: r.text, textTransform: 'uppercase', letterSpacing: '.06em', marginBottom: 4 }}>
                  {r.label} · {r.rango}
                </div>
                <input
                  ...
                  style={{ width: '100%', padding: '8px 10px', border: `1px solid ${r.border}`, borderRadius: 6, fontSize: 16, fontFamily: "'IBM Plex Mono', monospace", fontWeight: 600, background: '#fff', boxSizing: 'border-box' }}
                />
              </div>
            ))}
          </div>
          ...
          <div style={{ background: '#FDF0F0', border: '1px solid #F09595', borderRadius: 8, padding: '1rem', marginBottom: 16 }}>
            <div style={{ fontSize: 11, fontWeight: 600, color: '#7A1A1A', textTransform: 'uppercase', letterSpacing: '.06em', marginBottom: 4 }}>
              Menores de 200 kg → vuelven a acumulación
            </div>
            <input
              type="number" min="0" placeholder="0"
              value={form.menores}
              onChange={e => setForm({...form, menores: e.target.value})}
              style={{ width: '100%', padding: '8px 10px', border: '1px solid #F09595', borderRadius: 6, fontSize: 16, fontFamily: "'IBM Plex Mono', monospace", fontWeight: 600, background: '#fff', boxSizing: 'border-box' }}
            />
          </div>
          ...
          <div style={{ padding: '10px 12px', background: '#F7F5F0', borderRadius: 8, fontSize: 13, marginBottom: 16 }}>
            ...
            <span style={{ fontFamily: "'IBM Plex Mono', monospace", fontWeight: 600 }}>{totalIngresado} animales</span>
          </div>
  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '1.5rem' }}>
        <div>
          <h1 style={{ fontSize: 20, fontWeight: 600, marginBottom: 4 }}>Pesada y clasificación</h1>
          <div style={{ fontSize: 12, color: '#6B6760', fontFamily: "'IBM Plex Mono', monospace" }}>
            Próxima pesada fija: <strong>{proximaPesada}</strong> · cada 40 días
          </div>
        </div>
        <Btn onClick={() => setVista('nueva')}>+ Nueva pesada</Btn>
      </div>

      <div style={{ background: '#FDF0E0', border: '1px solid #EF9F27', borderRadius: 10, padding: '1rem 1.25rem', marginBottom: '1.25rem', fontSize: 13 }}>
        <strong style={{ color: '#7A4500' }}>Recordá:</strong> <span style={{ color: '#7A4500' }}>Rangos: A=200–230 kg · B=231–260 kg · C=261–290 kg · D=291+ kg · Objetivo de venta: 400 kg · Desbaste: 8%</span>
      </div>

      <Card titulo="Historial de pesadas">
        {pesadas.length === 0
          ? <p style={{ fontSize: 13, color: '#9E9A94', padding: '.5rem 0' }}>No hay pesadas registradas aún.</p>
          : pesadas.map(p => (
