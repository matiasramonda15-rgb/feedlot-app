...
export default function Ventas({ usuario }) {
  ...
  // Multi-corral
  const [const kgNetoP3 = Math.round(corralesVenta.reduce((s, c) => s + (parseFloat(c.kg_vivo) || 0), 0) * (1 - (parseFloat(form.desbaste) || 8) / 100))
const montoTotal = parseFloat(form.precio_kg || 0) * kgNetoP3, setCorralesVenta] = useState([{ corral_id: '', cantidad: '', kg_vivo: '' }])
  ...
  const kgNetoP3 = Math.round(kgBruto - kgDescuento)
  const precioKg = parseFloat(form.precio_kg) || 0
  const totalVenta = Math.round(kgNetoP3 * precioKg)
  ...
  async function guardarDatosVenta(venta) {
    ...
    if (venta.grupo_venta_id) {
      ...
      for (const v of (grupo || [])) {
        const kgNetoP3V = v.kg_vivo_total ? Math.round(v.kg_vivo_total * (1 - desbastePct / 100) * 100) / 100 : (v.kg_neto || 0)
