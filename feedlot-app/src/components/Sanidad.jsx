...
export default function Sanidad({ usuario, mobile, nav }) {
  ...
  const [formEventoM, setFormEventoM] = useState({ corral_id: '', productos: [{ prod_id: '', producto: '', dosis_ml: '5' }], cantidad: '', observaciones: '' })
  ...
  if (mobile) {
    ...
    async function registrarEventoM() {
      ...
      const productosValidos = formEventoM.productos.filter(p => p.prod_id)
      if (!productosValidos.length) { alert('Selecciona al menos un producto'); return }
      if (!formEventoM.cantidad) { alert('Ingresa la cantidad de animales'); return }
      setGuardandoM(true)
      const cantAnimales = parseInt(formEventoM.cantidad)
      // Un evento por cada producto aplicado — mismo criterio que en
      // "Revisión", así el historial muestra cada vacuna/producto por su
      // nombre real en vez de mezclarlos en un solo registro genérico.
      let mlTotalGeneral = 0
      for (const p of productosValidos) {
        const dosisMl = parseFloat(p.dosis_ml) || 0
        const mlTotal = dosisMl > 0 ? Math.round(cantAnimales * dosisMl) : null
        const prod = stockSanitarioM.find(x => String(x.id) === String(p.prod_id))
        const { error } = await registrarTratamientoSanitario(supabase, {
          tipo: 'tratamiento', corralId: parseInt(formEventoM.corral_id),
          productoId: prod?.id || null, productoNombre: p.producto,
          cantidadMl: mlTotal, cantidadAnimales: cantAnimales,
          observaciones: formEventoM.observaciones, usuario,
        })
        if (error) { alert(`Error al registrar ${p.producto}: ` + error.message); setGuardandoM(false); return }
        mlTotalGeneral += mlTotal || 0
      }
      await cargarDatos()
      alert(`Evento registrado.${productosValidos.length > 1 ? ` ${productosValidos.length} productos aplicados.` : mlTotalGeneral ? ` Se descontaron ${mlTotalGeneral.toLocaleString('es-AR')} ml.` : ''}`)
      setPantSan('alertas')
      setFormEventoM({ corral_id: '', productos: [{ prod_id: '', producto: '', dosis_ml: '5' }], cantidad: '', observaciones: '' })
      setGuardandoM(false)
    }
        ...
        <MobileScroll>
                      ...
                      if (vac.confirmada || yaVacunado) {
                        return (
                          <div style={{ background: '#1A3D26', border: `1px solid ${CM.green}`, borderRadius: 8, padding: '8px 12px', marginBottom: 8, fontSize: 12, color: CM.green }}>
                            ✓ Vacunado{(vac.resumen?.length > 0) ? ` — ${vac.resumen.map(r => `${r.nombre} ${r.dosis}ml/animal${r.cantidad && loteC && r.cantidad !== loteC.cantidad ? ` (${r.cantidad} de ${loteC.cantidad})` : ''}`).join(' · ')}` : ''}
                          </div>
                        )
                      }
                      return (
                          ...
                          {!expandido ? (
                            ...
                            <div style={{ background: '#1A1A1A', border: `1px solid ${CM.amber}`, borderRadius: 8, padding: '1rem' }}>
                              ...
                              {vacunas.length === 0 ? (
                                ...
                                <>
                                  <div style={{ fontSize: 11, color: CM.muted, marginBottom: 10 }}>Cada vacuna puede tener su propia cantidad de animales — por defecto, todo el lote ({loteC?.cantidad || 0}).</div>
                                  {vacSeleccionadas.map((vs, vi) => {
                                    const cantidadVac = vs.cantidad !== undefined && vs.cantidad !== '' ? parseInt(vs.cantidad) : (loteC?.cantidad || 0)
                                    const mlTotal = vs.prod_id && vs.dosis ? Math.round(cantidadVac * parseFloat(vs.dosis || 5)) : null
                                    return (
                                      <div key={vi} style={{ marginBottom: 14, paddingBottom: 10, borderBottom: vacSeleccionadas.length > 1 ? `1px dashed ${CM.border}` : 'none' }}>
                                        ...
                                        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                                          ...
                                          <div style={{ flex: 1 }}>
                                            <div style={{ fontSize: 11, color: CM.amber, marginBottom: 4 }}>Cant. animales</div>
                                            <input type="number" inputMode="numeric" min="1" max={loteC?.cantidad || 1} placeholder={String(loteC?.cantidad || '')} value={vs.cantidad ?? ''}
                                              onChange={e => {
                                                const nuevas = vacSeleccionadas.map((x, i) => i === vi ? {...x, cantidad: e.target.value} : x)
                                                setVacunacionLote(prev => ({...prev, [vacKey]: {...(prev[vacKey]||{}), vacunas: nuevas}}))
                                              }}
                                        ...
                                        {mlTotal && <div style={{ fontSize: 12, color: CM.green, marginTop: 4 }}>→ {mlTotal.toLocaleString('es-AR')} ml ({cantidadVac} × {vs.dosis} ml)</div>}
                                      </div>
                                    )
                                  })}
                                  <button onClick={() => {
                                    ...
                                    <button disabled={!vacSeleccionadas.some(vs => vs.prod_id) || vac.guardando}
                                      onClick={async () => {
                                        ...
                                        const vacunasParaGuardar = validas.map(vs => {
                                          ...
                                          const cantidadVac = vs.cantidad !== undefined && vs.cantidad !== '' ? parseInt(vs.cantidad) : loteC.cantidad
                                          return prod ? { productoId: prod.id, nombre: prod.producto, dosisMlPorAnimal: parseFloat(vs.dosis || 5), cantidadAnimales: cantidadVac } : null
                                        }).filter(Boolean)
                        ...
                        {isProtocolo && expandido && loteAlerta && (
                          <div style={{ borderTop: `1px solid ${CM.amber}`, paddingTop: '1rem' }}>
                            {vacunas.length === 0 ? (
                              ...
                              <>
                                <div style={{ fontSize: 11, color: CM.muted, marginBottom: 10 }}>Cada vacuna puede tener su propia cantidad de animales — por defecto, todo el lote ({loteAlerta.cantidad}).</div>
                                {vacSeleccionadas.map((vs, vi) => {
                                  const cantidadVac = vs.cantidad !== undefined && vs.cantidad !== '' ? parseInt(vs.cantidad) : loteAlerta.cantidad
                                  const mlTotal = vs.prod_id && vs.dosis ? Math.round(cantidadVac * parseFloat(vs.dosis || 5)) : null
                                  ...
                                  return (
                                    <div key={vi} style={{ marginBottom: 14, paddingBottom: 10, borderBottom: vacSeleccionadas.length > 1 ? `1px dashed ${CM.border}` : 'none' }}>
                                      ...
                                      <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                                        ...
                                        <div style={{ flex: 1 }}>
                                          <div style={{ fontSize: 11, color: CM.amber, marginBottom: 4 }}>Cant. animales</div>
                                          <input type="number" inputMode="numeric" min="1" max={loteAlerta.cantidad} placeholder={String(loteAlerta.cantidad)} value={vs.cantidad ?? ''}
                                            onChange={e => {
                                              const nuevas = vacSeleccionadas.map((x, i) => i === vi ? {...x, cantidad: e.target.value} : x)
                                              setVacunacionLote(prev => ({...prev, [loteAlerta.id]: {...(prev[loteAlerta.id]||{}), vacunas: nuevas}}))
                                            }}
          ...
          {pantSan === 'evento' && (
            <>
              ...
              <div style={{ marginBottom: '.85rem' }}>
                <div style={{ fontSize: 11, fontWeight: 600, color: CM.muted, textTransform: 'uppercase', marginBottom: 4 }}>Productos aplicados</div>
                {stockSanitarioM.length === 0
                  ...
                  : formEventoM.productos.map((p, pi) => (
                    <div key={pi} style={{ display: 'flex', gap: 6, marginBottom: 6, alignItems: 'center' }}>
                      <select value={p.prod_id}
                        onChange={e => {
                          const prod = stockSanitarioM.find(x => String(x.id) === e.target.value)
                          const n = { ...formEventoM, productos: formEventoM.productos.map((x, i) => i === pi ? { ...x, prod_id: e.target.value, producto: prod?.producto || '' } : x) }
                          setFormEventoM(n)
                        }}
                        style={{ flex: 1, background: CM.surface, border: `1px solid ${CM.border}`, borderRadius: 8, padding: '11px 12px', fontSize: 14, color: CM.text, fontFamily: CM.sans }}>
                        <option value="">— Producto —</option>
                        {stockSanitarioM.map(x => <option key={x.id} value={x.id}>{x.producto} · {(x.cantidad_ml || 0).toLocaleString('es-AR')} {x.unidad || 'ml'}</option>)}
                      </select>
                      <input type="number" inputMode="decimal" placeholder="ml" value={p.dosis_ml}
                        onChange={e => { const n = { ...formEventoM, productos: formEventoM.productos.map((x, i) => i === pi ? { ...x, dosis_ml: e.target.value } : x) }; setFormEventoM(n) }}
                        style={{ width: 60, background: CM.surface, border: `1px solid ${CM.border}`, borderRadius: 8, padding: '11px 6px', fontSize: 14, fontFamily: CM.mono, color: CM.amber, textAlign: 'center' }} />
                      {formEventoM.productos.length > 1 && (
                        <button onClick={() => { const n = { ...formEventoM, productos: formEventoM.productos.filter((_, i) => i !== pi) }; setFormEventoM(n) }}
                          style={{ background: 'transparent', border: 'none', color: CM.muted, cursor: 'pointer', fontSize: 16, padding: '0 4px' }}>✕</button>
                      )}
                    </div>
                  ))
                }
                {stockSanitarioM.length > 0 && (
                  <button onClick={() => setFormEventoM({ ...formEventoM, productos: [...formEventoM.productos, { prod_id: '', producto: '', dosis_ml: '5' }] })}
                    style={{ padding: '6px 10px', fontSize: 12, background: 'transparent', border: `1px solid ${CM.amber}`, color: CM.amber, borderRadius: 6, cursor: 'pointer' }}>
                    + Otro producto
                  </button>
                )}
              </div>
              ...
              {formEventoM.productos.some(p => p.prod_id && p.dosis_ml) && formEventoM.cantidad && (
                <div style={{ marginBottom: '.85rem' }}>
                  {formEventoM.productos.filter(p => p.prod_id && p.dosis_ml).map((p, pi) => {
                    const mlTotal = Math.round(parseInt(formEventoM.cantidad) * parseFloat(p.dosis_ml))
                    const prod = stockSanitarioM.find(x => String(x.id) === String(p.prod_id))
                    const stockActual = prod?.cantidad_ml || 0
                    const alcanza = stockActual >= mlTotal
                    return (
                      <div key={pi} style={{ background: alcanza ? '#1A2E1A' : '#2E1A1A', border: `1px solid ${alcanza ? CM.green : CM.red}`, borderRadius: 8, padding: '10px 12px', marginBottom: 6 }}>
                        <div style={{ fontSize: 13, fontWeight: 600, color: alcanza ? CM.green : CM.red }}>
                          {p.producto}: {mlTotal.toLocaleString('es-AR')} ml totales ({formEventoM.cantidad} animales × {p.dosis_ml} ml)
                        </div>
              ...
              <div key={l.id} style={{ background: S.surface, border: `1px solid ${enCuarentena ? '#EF9F27' : S.border}`, borderRadius: 10, padding: '1.25rem', marginBottom: '1rem' }}>
                ...
                {(() => {
                  ...
                  const resumenGuardado = eventosVacunacion.length > 0
                    ? eventosVacunacion.map(e => ({ nombre: e.producto, dosis: e.cantidad_animales ? +(e.cantidad_ml / e.cantidad_animales).toFixed(1) : null, mlTotal: e.cantidad_ml || 0, cantidad: e.cantidad_animales }))
                    : null
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        {confirmada ? (
                          <div style={{ fontSize: 12, color: S.muted }}>
                            {resumenMostrar.map(r => `${r.nombre} ${r.dosis ? `${r.dosis}ml/animal ` : ''}(${r.mlTotal.toLocaleString('es-AR')} ml${r.cantidad && r.cantidad !== l.cantidad ? ` · ${r.cantidad} de ${l.cantidad} animales` : ''})`).join(' · ')}
                            ...
                            {todosProductos.length === 0 ? (
                              ...
                              <>
                                <div style={{ fontSize: 12, color: S.muted, marginBottom: 8 }}>
                                  Seleccioná una o más vacunas, la dosis, y a cuántos animales se les puso cada una (por defecto, todo el lote). Se descontará del stock automáticamente.
                                </div>
                                {vacSeleccionadas.map((vs, vi) => {
                                  const prodSel = todosProductos.find(p => String(p.id) === String(vs.prod_id))
                                  const cantidadVac = vs.cantidad !== undefined && vs.cantidad !== '' ? parseInt(vs.cantidad) : l.cantidad
                                  const mlTotal = vs.prod_id && vs.dosis ? Math.round(cantidadVac * parseFloat(vs.dosis || 5)) : null
                                  return (
                                    <div key={vi} style={{ marginBottom: 8, paddingBottom: 8, borderBottom: vacSeleccionadas.length > 1 ? `1px dashed ${S.border}` : 'none' }}>
                                      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr auto', gap: 8, alignItems: 'flex-end' }}>
                                        ...
                                        <div>
                                          {vi === 0 && <div style={{ fontSize: 10, fontWeight: 600, color: S.amber, textTransform: 'uppercase', marginBottom: 3 }}>Cant. animales</div>}
                                          <input type="number" min="1" max={l.cantidad} placeholder={String(l.cantidad)} value={vs.cantidad ?? ''}
                                            onChange={e => {
                                              const nuevas = vacSeleccionadas.map((x, i) => i === vi ? {...x, cantidad: e.target.value} : x)
                                              setVacunacionLote(prev => ({...prev, [l.id]: {...(prev[l.id]||{}), vacunas: nuevas}}))
                                            }}
                                            style={{ width: '100%', padding: '8px 10px', border: `1px solid ${S.amber}`, borderRadius: 6, fontSize: 13, fontFamily: 'monospace', boxSizing: 'border-box' }} />
                                        </div>
                                      ...
                                      {vs.prod_id && vs.dosis && mlTotal && (
                                        <div style={{ marginTop: 4, fontSize: 11, color: S.accent }}>
                                          → {mlTotal.toLocaleString('es-AR')} ml de {prodSel?.n} ({cantidadVac} de {l.cantidad} animales × {vs.dosis} ml)
                                        </div>
                                      )}
                                    </div>
                                  )
                                ...
                                <button disabled={!vacSeleccionadas.some(vs => vs.prod_id) || vac.guardando}
                                  onClick={async () => {
                                    ...
                                    for (const vs of validas) {
                                      ...
                                      const cantidadVac = vs.cantidad !== undefined && vs.cantidad !== '' ? parseInt(vs.cantidad) : l.cantidad
                                      const mlDesc = Math.round(cantidadVac * dosis)
                                      ...
                                      vacunasParaGuardar.push({ productoId: prod.id, nombre: prod.n, dosisMlPorAnimal: dosis, cantidadAnimales: cantidadVac })
                                    }
